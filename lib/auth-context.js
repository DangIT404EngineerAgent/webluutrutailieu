...
export function AuthProvider({ children }) {
  ...
  // Lưu user ID hiện tại để so sánh, tránh re-render không cần thiết
  ...
  useEffect(() => {
    ...
    const handleAuthUser = async (sessionUser) => {
      ...
      if (sessionUser) {
        ...
        // CHỈ update state nếu user ID thực sự thay đổi
        // → Tránh re-render toàn bộ app khi token refresh
        if (currentUserIdRef.current !== newUserId) {
          ...
          const profileData = await fetchProfile(newUserId)
          if (isMounted) {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(
      async (event, session) => {
        ...
        // Chỉ xử lý events quan trọng: đăng nhập, đăng xuất, đổi user
        // Bỏ qua TOKEN_REFRESHED nếu cùng user → tránh re-render không cần thiết
        if (event === 'TOKEN_REFRESHED') {
          // Token refresh không đổi user, chỉ cần đảm bảo loading = false
          if (isMounted) setLoading(false)
          return
        }
